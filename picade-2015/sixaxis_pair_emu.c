...
#define BUTTON1   !(PINF & _BV(PF0))
#define BUTTON2   !(PINF & _BV(PF1))
#define BUTTON3   !(PINF & _BV(PF4))
#define BUTTON4   !(PINF & _BV(PF5))
#define BUTTON5   !(PINF & _BV(PF6))
#define BUTTON6   !(PINF & _BV(PF7))

/* NO GPIO on new picade
...
#define JOY_UP    !(PIND & _BV(PD7))
#define JOY_DOWN  !(PINB & _BV(PB4))
#define JOY_LEFT  !(PINB & _BV(PB6))
#define JOY_RIGHT !(PINB & _BV(PB5))

#define UTIL_ESC  !(PIND & _BV(PD3))
#define UTIL_ENT  !(PIND & _BV(PD2))
#define UTIL_SEL  !(PIND & _BV(PD4))
#define UTIL_STA  !(PIND & _BV(PD6))

#define VOL_UP    !(PIND & _BV(PD0))
#define VOL_DOWN  !(PIND & _BV(PD1))

#define PORTD_MASK 0b11011111
#define PORTB_MASK 0b01110000
...
*/
  ...
  //if(){
  		joystick_mode = !BUTTON3;
  //}

  if(joystick_mode){
	  RefReportData.buffer[2] |= (JOY_LEFT  << 7);
	  RefReportData.buffer[2] |= (JOY_DOWN  << 6);
	  RefReportData.buffer[2] |= (JOY_RIGHT << 5);
	  RefReportData.buffer[2] |= (JOY_UP    << 4);

  /* Goddamn analog button data */
  RefReportData.buffer[14] = (JOY_UP)*255; // Down
  RefReportData.buffer[15] = (JOY_RIGHT)*255; // Right
  RefReportData.buffer[16] = (JOY_DOWN)*255; // Up
  RefReportData.buffer[17] = (JOY_LEFT)*255; // Left
  ...
  else
  {
	  ...
	  if( JOY_UP )    RefReportData.buffer[7] = 0;
	  if( JOY_DOWN )  RefReportData.buffer[7] = 255;
	  if( JOY_LEFT )  RefReportData.buffer[6] = 0;
	  if( JOY_RIGHT ) RefReportData.buffer[6] = 255;

  		/*RefReportData.buffer[6] = 127+((!JOY_UP*127)   + (!JOY_DOWN*-127));
  		RefReportData.buffer[7] = 127+((!JOY_LEFT*127) + (!JOY_RIGHT*-127));*/
  }

  RefReportData.buffer[2] |= (UTIL_STA  << 3); // Start
  RefReportData.buffer[2] |= (UTIL_SEL  << 0); // Select

  RefReportData.buffer[3] |= (BUTTON2   << 4); // a / triangle 
  RefReportData.buffer[3] |= (BUTTON1   << 5); // b / circle
  RefReportData.buffer[3] |= (BUTTON4   << 6); // x / cross
  RefReportData.buffer[3] |= (BUTTON6   << 7); // y / square



  RefReportData.buffer[25] = (BUTTON5)*255; // Square
  RefReportData.buffer[24] = (BUTTON4)*255; // Cross
  RefReportData.buffer[23] = (BUTTON1)*255; // Circle
  RefReportData.buffer[22] = (BUTTON2)*255; // Triangle
  

  //RefReportData.buffer[4] |= (BUTTON5   << 0); // guide
  RefReportData.buffer[4] |= (BUTTON6   << 0); // guide
...
*/
  ...
  RefReportData.buffer[21] = (UTIL_ESC)*255; // R1
  RefReportData.buffer[3] |= (UTIL_ESC << 3); // Right Stick / 

  RefReportData.buffer[20] = (VOL_DOWN)*255; // L1
  RefReportData.buffer[3] |= (VOL_DOWN << 2); // right bumper /

  RefReportData.buffer[19] = (UTIL_ENT)*255;   // R2
  RefReportData.buffer[3] |= (UTIL_ENT << 1); // left bumper / 

  RefReportData.buffer[18] = (VOL_UP)*255; // L2
  RefReportData.buffer[3] |= (VOL_UP << 0); // Left Stick / 
